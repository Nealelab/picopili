...
my $readrefsum_script = "readrefsum_pico_chr23.pl";  ### my.pipeline_tar
my $buigue_script = "buigue_pico_chr23.pl";              ### my.pipeline_tar
...
foreach my $scr_name (@test_scripts) {
    ...
    my $path = "/stanley/genetics/users/nbaya/spark/array_May2019/spark_ricopili/imputation/impute_chr23/picopili_chr23/bin";
...
#########
my $chrloc = 23; # only look at chr 23
...
unless (-e "$rootdir/buigue_done") {
    unless (-e "$rootdir/posing_done") {
    		my $path = "/stanley/genetics/users/nbaya/spark/array_May2019/spark_ricopili/imputation/impute_chr23/picopili_chr23/bin"; # path to scripts
		foreach (@bim_files) {
		    my $bfile = $_;
		    $bfile =~ s/.bim$//;
	...
	#	    exit;
		    unless (exists $bimhg19_array{$accfli}) {
				push @buigue_arr, "$path/$buigue_script --lift19 $bfile.bim" ;#
		    }
		    else {
				$buigue_fini++;
		    }
...
unless (-e "$rootdir/readref_done") {
	my $path = "/stanley/genetics/users/nbaya/spark/array_May2019/spark_ricopili/imputation/impute_chr23/picopili_chr23/bin"; # path for scripts
	foreach (@bim_files) {
	    my $bimfile = $_;
	    my $bfile = $bimfile;
	    $bfile =~ s/.bim$//;
	    ...
	    unless (exists $bimref_array{$bimref}) {
	    		push @readref_arr, "$path/$readref_script --ref $reffi --refheads id,NULL,position,a1,a0,$popname_uc --chr $chrloc $accfli" ;#
...
#		print "looking for $bimref_done\n";
		unless (exists $bimref_array{$bimref_done}) {
		    push @readrefsum_arr, "$path/$readrefsum_script $accfli" ;#
		}
		else {
		    $readrefsum_fini++;
		}
...
unless (-e "$rootdir/posing_done") {
	...
	if (-e $locref) {
		...
		push @chepos_arr, "$path/$checkpos_script --dbcol 1,2,3 --dbsnp $rootdir/$impute_dir/$locref $bfile.hg19.bim" ;#
	...
	foreach (@bim_files) {
	    ...
	    if (-e $locref) {
			...
			unless (exists $bimfli_array{$accfli}) {
				my $systmp = "$path/$checkflip_script --dbcol 0,3,4,5 --fth $fth_th --sfh $sec_freq --info $rootdir/$impute_dir/$locref $bfile.hg19.ch.bim" ;
