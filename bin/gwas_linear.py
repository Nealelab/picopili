...
####################################
# gwas_linear.py
# Raymond Walters, October 2018
"""
Runs GWAS of plink data using linear regression
"""
# Overview:
...
parser = argparse.ArgumentParser(prog='gwas_linear.py',
...
if args.addout is not None:
    outstem = 'linear.' + str(args.out) + '.' + str(args.addout)
else:
    outstem = 'linear.' + str(args.out)
...
# check proper output 
assert file_len(outstem +'.assoc.linear') >= nsnp, 'Failed to generate full GWAS results file %s' % outstem +'.assoc.linear'
